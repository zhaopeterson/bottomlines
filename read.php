...
if (isset($_GET['tid']) && is_numeric($_GET['tid'])) {
	...
	if ($tid > 0) { // Check against the database...
		...
		// Run the query:
		$q = "SELECT t.subject, p.message, username, DATE_FORMAT($posted, '%b-%e-%y %l:%i %p') AS posted FROM threads AS t LEFT JOIN posts AS p USING (thread_id) INNER JOIN users AS u ON p.user_id = u.user_id WHERE t.thread_id = $tid ORDER BY p.posted_on ASC";
